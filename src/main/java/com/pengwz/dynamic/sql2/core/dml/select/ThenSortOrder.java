...
public class ThenSortOrder<R> implements Fetchable {
    ...
    public ThenSortOrder(TableRelation<R> tableRelation, Fn<?, ?> fn, SortOrder sortOrder) {
        this.tableRelation = tableRelation;
        tableRelation.getSelectSpecification().getOrderBys().add(new DefaultOrderBy(fn, sortOrder));
    }

    public ThenSortOrder(TableRelation<R> tableRelation, String orderingFragment) {
        this.tableRelation = tableRelation;
        tableRelation.getSelectSpecification().getOrderBys().add(new CustomOrderBy(orderingFragment));
    }

    public <T, F> ThenSortOrder<R> thenOrderBy(Fn<T, F> orderByValue) {
        return thenOrderBy(orderByValue, SortOrder.ASC);
    }

    public <T, F> ThenSortOrder<R> thenOrderBy(Fn<T, F> orderByValue, SortOrder sortOrder) {
        tableRelation.getSelectSpecification().getOrderBys().add(new DefaultOrderBy(orderByValue, sortOrder));
        return this;
    }

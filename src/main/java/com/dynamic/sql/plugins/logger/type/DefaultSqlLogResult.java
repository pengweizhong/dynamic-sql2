...
public class DefaultSqlLogResult extends AbstractSqlLog implements SqlLogResult {

    @Override
    public void afterLog(SqlLogProperties props, SqlLogContext ctx) {
        SqlDebugger.printSql(props.getLevel(), "{} <--       Result: {}", getPrintDataSourceName(props, ctx), toLogValue(ctx.getRawResult()));
    }
}
